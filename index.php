...
if($companyFilter){
    $companies = array_filter($companies, fn($c)=>$c->name === $companyFilter);
}
    ...
    foreach($companies as $key => $company){
        ...
        foreach($company->contacts as $contact){
            ...
            foreach($contact->meetings as $m){
                if(stripos($m->topic, $search) !== false){
                    $matchMeeting = true;
                    break;
                }
            }
            if($matchContact || $matchMeeting){
                $filteredContacts[] = $contact;
            }
        }
        $companies[$key]->contacts = $filteredContacts;
    }
    $companies = array_filter($companies, fn($c)=>count($c->contacts) > 0);
}
